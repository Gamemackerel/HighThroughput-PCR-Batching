# This file defines many general use helper methods for use by the GradientPcrBatching
# and GradientPcrRepresentation machinery.
# These include convienent numeric comparisions, heap methods,
...
def build_mst_adjacency_list(graph, nodelist)
    ...
    for i in 1...nodelist.size do #O(n)
        ...
        adjacency_list.push(pair, graph[i][j]) unless adjacency_list.has_key?(pair) #O(1)
    end
    adjacency_list
end
...
# O(n^2)
# @param graph [Array<Array<Float>>]  matrix holding the distance of every
#                        pair of nodes in the graph, with a distance of -1
#                        for self-pairings
# @return [Array<Integer>]  disjoint-set forest where each index is the item id
#                        and each element is a  parent pointer that can be used
#                        to traverse back up the path travelled
def prim(graph)
    n = graph.size
    parent = Array.new(n)
    key = Array.new(n)
